...
IP_OUT2="udp:192.168.0.195:"  # IP do computador rodando o QGC
...
"$SIMV_BIN" -D \
  ...
  --out=${IP_OUT1}${ext_port} \
  --out=${IP_OUT1}${QGC_PORT} \
  --out=${IP_OUT2}${ext_port} \
  --out=${IP_OUT2}${QGC_PORT} \
  --out=${IP_OUT3}${ext_port} \
  --out=${IP_OUT3}${QGC_PORT} \
  -L "${LOCATION}" -f ${FRAME} --model ${MODEL} \
  -I ${inst} --sysid ${sysid} \
  --console --add-param-file ${PARAM_FILE} \
  2>&1 | tee -a "$log" ;
echo; echo "Log: $log";
echo "[SITL] Instância ${i}  (I=${inst}, SYSID=${sysid})  QGC:${QGC_PORT}  MAVROS:${mav_port}  ext:${IP_OUT1}${IP_OUT2}${IP_OUT2}${ext_port}"
echo "Finalizado. Pressione ENTER para fechar.";
read
EOF
)

  echo "[SITL] Instância ${i}  (I=${inst}, SYSID=${sysid})  QGC:${QGC_PORT}  MAVROS:${mav_port}  ext:${IP_OUT1}${ext_port}"
